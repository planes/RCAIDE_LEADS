...
def Harmonic_Noise_Validation(PP):
    ...
    theta                   = rp.array([1,10,20,30.1,40,50,59.9,70,80,89.9,100,110,120.1,130,140,150.1,160,170,179])  
    S                       = 20 # 4.
    test_omega              = rp.array([2390,2710,2630]) * Units.rpm    
    ctrl_pts                = len(test_omega)
    AoA                     = rp.zeros(ctrl_pts)
    true_course             = 0
    fligth_path_angle       = 0 
    ...
    segment                                                = Segment()  
    conditions                                             = Results() 
    conditions.aeroacoustics.relative_microphone_locations         = rp.repeat(mic_positions[ rp.newaxis,:,: ],1,axis=0)   
    conditions.aerodynamics.angles.alpha                   = rp.atleast_2d(AoA).T 
    conditions.freestream.density                          = rp.ones((ctrl_pts,1)) * density
    conditions.freestream.dynamic_viscosity                = rp.ones((ctrl_pts,1)) * dynamic_viscosity   
    conditions.freestream.speed_of_sound                   = rp.ones((ctrl_pts,1)) * a 
    conditions.freestream.temperature                      = rp.ones((ctrl_pts,1)) * T  
    ...
    conditions.frames.wind.transform_to_inertial[:,0,0]    = rp.cos(fligth_path_angle) 
    conditions.frames.wind.transform_to_inertial[:,0,2]    = rp.sin(fligth_path_angle) 
    conditions.frames.wind.transform_to_inertial[:,1,1]    = 1 
    conditions.frames.wind.transform_to_inertial[:,2,0]    = -rp.sin(fligth_path_angle) 
    conditions.frames.wind.transform_to_inertial[:,2,2]    = rp.cos(fligth_path_angle)  
    conditions.frames.body.transform_to_inertial           = rp.zeros((ctrl_pts,3,3))
    conditions.frames.body.transform_to_inertial[:,0,0]    = rp.cos(AoA)
    conditions.frames.body.transform_to_inertial[:,0,2]    = rp.sin(AoA)
    conditions.frames.body.transform_to_inertial[:,1,1]    = 1
    conditions.frames.body.transform_to_inertial[:,2,0]    = -rp.sin(AoA)
    conditions.frames.body.transform_to_inertial[:,2,2]    = rp.cos(AoA)     
    ...
    for fid in  range(len(fidelities)): 
        ...
        error.SPL_Case_1_60deg  = rp.max(rp.abs(F8745D4_SPL[0,6] - F8745_D4_verification_values_60deg[fid])/F8745_D4_verification_values_60deg[fid])  
        error.SPL_Case_1_90deg  = rp.max(rp.abs(F8745D4_SPL[0,9] - F8745_D4_verification_values_90deg[fid])/F8745_D4_verification_values_90deg[fid])
...
def Broadband_Noise_Validation(PP):
    APC_broadboand_verification_60deg = rp.array([15.12527918, 17.05865653, 19.35486775, 21.70880183, 24.34934545,
                                                  ...
                                                  54.40820869])
    APC_broadboand_verification_90deg = rp.array([21.28334833, 23.21667437, 25.51424316, 27.87113539, 30.51433249,
    ...
    T                             = 286.16889478                       
    APC_SF_RPM                    = rp.array([3600,4200,4800])
    ...
    theta                         = rp.array([45., 67.5, 90.001, 112.5 , 135.])  # rp.linspace(0.1,180,100)  
    S                             = 1.905
    AoA                           = rp.zeros(ctrl_pts)
    true_course                   = 0
    fligth_path_angle             = 0    
    ...
    conditions.freestream.speed_of_sound                   = rp.ones((ctrl_pts,1)) * a 
    conditions.freestream.temperature                      = rp.ones((ctrl_pts,1)) * T
    v_mat                                                  = rp.zeros((ctrl_pts,3))
    v_mat[:,0]                                             = velocity 
    conditions.frames.inertial.velocity_vector             = v_mat 
    conditions.energy.throttle                             = rp.ones((ctrl_pts,1)) * 1.0    
    conditions.aerodynamics.angle_of_attack                = rp.atleast_2d(AoA).T  
    conditions.freestream.mach_number                      = rp.atleast_2d(rp.linalg.norm(conditions.frames.inertial.velocity_vector,axis = 1)).T/ a
    conditions.frames.planet.true_course                   = rp.zeros((ctrl_pts,3,3)) 
    conditions.frames.planet.true_course[:,0,0]            = rp.cos(true_course),
    conditions.frames.planet.true_course[:,0,1]            = - rp.sin(true_course)
    ...
    error.SPL_Case_1_60deg  = rp.max(rp.abs(APC_SF_SPL_broadband_1_3_spectrum[1,3,8:]  - APC_broadboand_verification_60deg)/APC_broadboand_verification_60deg)
    error.SPL_Case_1_90deg  = rp.max(rp.abs(APC_SF_SPL_broadband_1_3_spectrum[1,4,8:] - APC_broadboand_verification_90deg)/APC_broadboand_verification_90deg)
...
def Hararmonic_Noise_Validation_Data(PP):  
    ...
    validation_data.harmonics              = rp.arange(1,19)
    validation_data.ANOPP_PAS_Case_1_60deg = rp.array([105.82,101.08,100.13,97.581,94.035,89.095,82.957,
                                       80.609,81.052,72.718,70.772,68.023,67.072,53.949,
                                       rp.inf,rp.inf,rp.inf,rp.inf]) 

    validation_data.Exp_Test_Case_1_60deg  = rp.array([103.23,100.08,98.733,94.990,91.045,87.500,82.161,79.012,
                                       74.469,70.128,65.784,61.241,56.699,51.958,47.013,42.673,
                                       37.927,32.989,]) 

    validation_data.ANOPP_PAS_Case_1_90deg = rp.array([107.671,104.755,105.829,103.307,101.385,100.465,99.1399,
                                       96.8208,93.6988,91.7765,89.6573,86.5323,85.2098,83.4874,
                                       78.1692,75.4503,73.7248,72.0024]) 

    validation_data.Exp_Test_Case_1_90deg  = rp.array([108.077,107.554,105.626,103.307,100.988,100.068,98.9430,
                                       96.8208,93.6988,91.7796,89.6542,85.5295,85.0099,81.8879,
                                       77.9724,74.8566,73.1250,71.2057  ])    

    validation_data.Exp_Test_Case_2_60deg = rp.array([111.951,108.175,108.789,106.352,105.059,100.140,100.945,
                                      99.8430,93.9683,93.8203,91.1914,85.3167,85.3626,82.1580,
                                       78.1933,75.7552,80.1887,72.2133])

    validation_data.ANOPP_PAS_Case_2_60deg = rp.array([111.760,111.421,108.984,106.352,104.487,101.856,99.0369,
                                       95.4522,93.2050,89.4327,86.2313,82.6498,79.2559,75.4804,
                                       71.1356,68.1219,63.9663,60.0000]) 


    validation_data.Exp_Test_Case_2_90deg  = rp.array([115.587,113.363,115.520,113.868,113.365,111.331,
                                       113.491,110.507,109.999,109.873,107.649,106.949,
                                       106.822,103.079,103.715,102.633,99.6502,97.8095])

    validation_data.ANOPP_PAS_Case_2_90deg = rp.array([115.397,115.273,114.377,113.870,113.362,111.143,
                                       110.631,109.752,108.859,107.585,109.175,105.234,
                                       103.782,102.127,101.236,99.7790,98.7002,98.9523  ])  

    validation_data.Exp_Test_Case_3_60deg  = rp.array([110.93,107.28,108.60,106.28,104.17,99.377,
                                       100.69,100.28,95.688,95.094,92.975,84.365,
                                       84.533,82.224,77.622,77.411,78.152,74.312])
    validation_data.ANOPP_PAS_Case_3_60deg = rp.array([110.93,110.53,108.41,107.43,104.55,101.47,
                                       98.592,95.328,92.635,88.987,86.103,83.028,
                                       79.573,76.114,73.040,69.775,65.554,61.908 ])

    validation_data.Exp_Test_Case_3_90deg  = rp.array([114.499,112.135,114.674,112.898,112.299,111.308,
                                       112.473,110.894,109.510,109.303,107.724,107.124,
                                       106.133,102.790,103.758,101.983,99.2279,98.0404])

    validation_data.ANOPP_PAS_Case_3_90deg = rp.array([114.499,114.291,113.889,113.879,111.122,110.523,
    ...
    axes_2_1.set_yticks(rp.arange(50,150,25))     
    axes_2_1.grid(True)  
...
def Broadband_Noise_Validation_Data(PP):  
    validation_data = Data()      
    validation_data.Exp_APC_SF_freqency_spectrum =  rp.array([100, 125, 160, 200, 250, 315, 400, 500, 630, 800, 1000, 1250, 1600, 
                                              2000, 2500, 3150,4000, 5000, 6300, 8000, 10000])   

    validation_data.Exp_APC_SF_1_3_Spectrum      = rp.array([[22.149, 42.242, 24.252, 22.616, 26.121, 24.953, 28.925, 29.158, 39.205
    ...
    validation_data.Exp_broadband_APC = rp.array([[24.8571428,27.7142857,28.8571428,26.5714285,27.1428571,27.7142857,30.2857142,32,35.4285714,
...
def setup_noise_settings(sts): 
    ...
    sts.ground_microphone_phi_angles   = rp.array([30.,45.,60.,75.,89.9,90.1,105.,120.,135.,150.])*Units.degrees
    sts.ground_microphone_theta_angles = rp.array([89.9,89.9,89.9,89.9,89.9,89.9,89.9,89.9, 89.9,89.9 ])*Units.degrees
    sts.center_frequencies             = rp.array([16,20,25,31.5,40, 50, 63, 80, 100, 125, 160, 200, 250, 315, 400, \
                                                   500, 630, 800, 1000, 1250, 1600, 2000, 2500, 3150,
                                                   4000, 5000, 6300, 8000, 10000])        
    sts.lower_frequencies              = rp.array([14,18,22.4,28,35.5,45,56,71,90,112,140,180,224,280,355,450,560,710,\
                                                   900,1120,1400,1800,2240,2800,3550,4500,5600,7100,9000 ])
    sts.upper_frequencies              = rp.array([18,22.4,28,35.5,45,56,71,90,112,140,180,224,280,355,450,560,710,900,1120,\
                                                   1400,1800,2240,2800,3550,4500,5600,7100,9000,11200 ])
    sts.harmonics                      = rp.arange(1,30)


    sts.broadband_spectrum_resolution        = 301
    sts.floating_point_precision             = rp.float32
