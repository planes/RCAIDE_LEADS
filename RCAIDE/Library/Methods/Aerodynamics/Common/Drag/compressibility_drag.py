...
def compressibility_drag(state,settings,geometry):
    ...
    **Definitions**
    ...
    cd_compressibility  = rp.zeros_like(Mach)
    ...
    for wing in  geometry.wings:
        if isinstance(wing,RCAIDE.Library.Components.Wings.Main_Wing) or isinstance(wing,RCAIDE.Library.Components.Wings.Blended_Wing_Body):
            ...
            tc = wing.thickness_to_chord / rp.cos(sweep_w)
            cl = conditions.aerodynamics.coefficients.lift.total/ (rp.cos(sweep_w) ** 2)
...
def transonic_lift_wave_drag(conditions, settings, geometry): 
    ...
    **Definitions**
    ...
    CD_wave_transonic = rp.zeros_like(Mach)
    
    if settings.use_surrogate or (settings.vortex_distribution == None): 
        Cl                   = conditions.aerodynamics.coefficients.lift.total
        CD_wave_transonic    = rp.array([-1.34E-03,2.35E-05,1.42E-03,1.95E-03,2.23E-03,2.56E-03,2.80E-03, 4.32E-03,4.89E-03,
                                         7.11E-03,1.48E-02, 2.31E-02,2.83E-02,3.42E-02,3.96E-02,3.76E-02,2.71E-02]) 
        CLs                  = rp.array([-0.25836715,-0.05233014,0.08334449,0.21608904,0.35012533, 0.42120447,0.48458659,
                                         0.55214519,0.62108179,0.69313687,0.75284432,0.81921256,0.95662739,1.07911642,1.19681914,1.33118394,1.47947542      ])    
        CD_wave_transonic    = rp.interp(Cl, CLs, CD_wave_transonic) 
    else: 
        ...
        CD_wave_total = rp.zeros_like(Mach)
        ...
        M1_0_star = rp.sqrt((5+Mach**2*rp.cos(delta[0])**2)/(1+0.7*Mach**2*Cp_shock)**(2/7) - 5)
        
        # Calculate the wave drag coefficient
        constant = (rp.cos(delta[0])**4)/c_kappa * 0.243*((1+0.2*Mach*rp.cos(delta[0]))/(Mach*rp.cos(delta[0])))**3 # note, delta is assumed to remain constant across all conditions. 
        ...
        CD_wave_total = rp.sum(CD_wave_segment*S_segment/S_ref, axis=1, keepdims=True)
...
def supersonic_lift_wave_drag(conditions,configuration,geometry):
    ...
    **Definitions**
    ...
    # Initalize cd arrays 
    Mach         = conditions.freestream.mach_number
    cd_lift_wave = rp.zeros_like(Mach)
    ...
    cd_lift_wave[rp.isnan(cd_lift_wave)] = 0.  
    return cd_lift_wave
...
def supersonic_volume_wave_drag(conditions, settings, vehicle):
    ...
    **Definitions**
    ...
    for fuselage in vehicle.fuselages:
        Rmax =  rp.maximum(Rmax, fuselage.width)
    volume      =  (3 / 16) * (rp.pi ** 2) * (Rmax ** 2)  *  L 
    CD_wave_vol_frontal = 24 *volume /(L **3)
    
    CD_wave_vol =  CD_wave_vol_frontal * ( Amax / S_ref) *  rp.ones_like(conditions.freestream.mach_number) 
    return CD_wave_vol
