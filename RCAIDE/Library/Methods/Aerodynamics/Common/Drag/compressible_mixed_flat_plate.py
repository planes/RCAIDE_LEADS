...
def compressible_mixed_flat_plate(Re,Ma,Tc,xt):
    ...
    **Definitions**
    ...
    #cf_turb  = 0.455/(rp.log10(Rext)**2.58) # Schlichting empircal formula for turbulent flat plate. note that this correlation is meant for smooth surfaces.
    ...
    y = rp.array([0.01273, 0.0090077, 0.0058721317, 0.00716019, 0.0049416, 0.003744045, 0.00316321, 0.002506763, 0.002064, 0.0016604122])
    x = rp.array([10000, 115300, 675100, 281300, 1492600, 6040200, 14920400, 58584300, 193593000, 897594100])
    cf_turb = rp.interp(Rext, x, y) # Data is from Figure 11.2 of Shevell's Fundamentals of Flight.
    cf_lam   = 1.328/(Rex**0.5)
    
    if xt > 0.0:
        cf_start = rp.interp(Re*xeff, x, y)  
    else:
        cf_start = 0.0
