...
def chordwise_distribution(x,y,cp,al,npanel,cf,vt):
   ...
   Properties Used:
    ...
    dy       = y[1:]-y[:-1]
    LE_ind   = int(npanel/2)
    dx_l     = -rp.flip(dx[0:LE_ind], axis=0)
    dx_u     = dx[LE_ind:]
    dy_l     = -rp.flip(dy[0:LE_ind], axis=0)
    dy_u     = dy[LE_ind:]
    
    cp_l     = rp.flip(cp[0:LE_ind], axis=0)
    cp_u     = cp[LE_ind:]
    
    cf       = cf*(vt/rp.abs(vt))
    cf_l     = -rp.flip(cf[0:LE_ind], axis=0)
    cf_u     = cf[LE_ind:]
    
    dcn      = cp_l*dx_l - cp_u*dx_u + cf_u*dy_u - cf_l*dy_l
    dca      = cp_u*dy_u - cp_l*dy_l + cf_u*dx_u + cf_l*dx_l
    
    dcl      = dcn*rp.cos(al) - dca*rp.sin(al)
    dcd      = dcn*rp.sin(al) + dca*rp.cos(al)
    
    cl       = rp.sum(dcl, axis=0)
    cd       = rp.sum(dcd, axis=0)
