...
def get_seat_y_coords(cabin,cabin_class,cabin_class_origin):
    ...
    if n == 1:
        if total_aisle_width == 0.0: 
            s_y_coord = rp.array([0])            
        else: 
            s_y_coord = rp.array([ccw/ 2 - ar_w - s_w /2])
    elif n == 2:
        y_1 = ccw/ 2 - ar_w - s_w /2
        y_2 = -y_1
        s_y_coord = rp.array([y_1, y_2 ])
    elif n == 3:
        y_1 =  ccw / 2 - s_w /2 - ar_w
        y_2 = -y_1
        y_3 =  y_1 - s_w - ar_w
        s_y_coord = rp.array([ y_1, y_2, y_3 ])
    elif n == 4:
        if cabin.wide_body:
            y_1 = ccw / 2 - s_w /2 - ar_w
            y_2 = -y_1
            y_3 =  s_w /2 + ar_w
            y_4 = -y_3
            s_y_coord = rp.array([ y_1, y_2, y_3,y_4 ])
        else:
            y_1 = ccw / 2 - s_w /2 - ar_w
            y_2 = -y_1
            y_3 = y_1 - s_w - ar_w
        ...
        y_4 =  -y_3
        y_5 =  y_4 + s_w + ar_w
        s_y_coord = rp.array([ y_1, y_2, y_3,y_4,y_5 ])
    elif n == 6:
        y_1 =  ccw / 2 - s_w /2 - ar_w
        y_2 =  -y_1
        y_3 =  y_1 - s_w - ar_w
        ...
        y_5 =  y_3 - s_w - ar_w
        y_6 = -y_5
        s_y_coord = rp.array([ y_1, y_2, y_3,y_4, y_5,y_6 ])
    elif n == 7:
        y_1 =  ccw / 2 - s_w /2 - ar_w
        y_2 =  -y_1
        y_3 =  y_1 - s_w - ar_w
        ...
        y_6 =  -y_5
        y_7 = 0
        s_y_coord = rp.array([ y_1, y_2, y_3,y_4, y_5,y_6,y_7])
    elif n == 8:
        y_1 =  ccw / 2 - s_w /2 - ar_w
        y_2 =  -y_1
        y_3 =  y_1 - s_w - ar_w
        ...
        y_7 = y_5 + s_w + ar_w
        y_8 = -y_7
        s_y_coord = rp.array([ y_1, y_2, y_3,y_4, y_5,y_6,y_7,y_8 ])
    elif n == 9:
        y_1 =  ccw / 2 - s_w /2 - ar_w
        y_2 =  -y_1
        y_3 =  y_1 - s_w - ar_w
        ...
        y_8 =  -y_7
        y_9 = 0
        s_y_coord = rp.array([ y_1, y_2, y_3,y_4, y_5,y_6,y_7,y_8,y_9])
    elif n == 10:
        y_1 =  ccw / 2 - s_w /2 - ar_w
        y_2 =  -y_1
        y_3 =  y_1 - s_w - ar_w
...
def get_seat_x_coords(cabin,cabin_class,cabin_class_origin,cabin_length):
    ...
    for i in range(len(gl_loc)):
        ...
        if loc == (n_r -1):
            ...
            s_x_coord = rp.append( s_x_coord,new_loc  )
...
#     theta1 = rp.arctan(cabin_width/nose_length)
#     n_idxs =  rp.where(nose_length > seat_data[:,2])[0]
#     removed_indexes =  []
...
#         x_pts = rp.array([x0, x1])
#         y_pts = rp.array([y0, y1])
#         y_border = max(x_pts * rp.tan(theta1))
#         if rp.any( y_pts > y_border):
#             removed_indexes.append(n_idxs[n_i])
#     seat_data = rp.delete(seat_data,(removed_indexes), axis=0)
#     # remove components that fall outside of tapered tail region
#     theta2     = rp.arctan(cabin_width/tail_length)
#     tail_start = seat_data[-1,2] -  tail_length
#     t_idxs     = rp.where(tail_start < seat_data[:,2])[0]
#     removed_indexes =  []
...
#         x_pts = rp.array([x0, x1])
#         y_pts = rp.array([y0, y1])
#         y_border = max(x_pts * rp.tan(theta2))
#         if rp.any( y_pts > y_border): # or remove_feature == True:
...
def update_seat_map_layout_using_cabin_taper(seat_data, cabin):
    ...
    # -----------------------------
    # Remove components in tapered nose
    # -----------------------------
    theta1 = rp.arctan(cabin_width / nose_length)

    # seats whose x-center is inside the nose region
    n_idxs = rp.nonzero(seat_data[:, 2] < nose_length)[0]

    removed = []
    for k in range(len(n_idxs)):
        idx = n_idxs[k]

        x0 = seat_data[idx, 2] - seat_data[idx, 5] / 2
        x1 = seat_data[idx, 2] + seat_data[idx, 5] / 2
        y0 = seat_data[idx, 3] - seat_data[idx, 6] / 2
        y1 = seat_data[idx, 3] + seat_data[idx, 6] / 2

        x_pts = rp.array([x0, x1])
        y_pts = rp.array([y0, y1])

        y_border = rp.max(x_pts * rp.tan(theta1))

        if rp.any(y_pts > y_border):
            removed.append(int(idx))

    if len(removed) !=0:
        seat_data = rp.delete(seat_data, rp.array(removed), axis=0)

    # -----------------------------
    # Remove components in tapered tail
    # -----------------------------
    theta2 = rp.arctan(cabin_width / tail_length)

    # tail starts tail_length behind last seat
    tail_start = seat_data[-1, 2] - tail_length

    t_idxs = rp.nonzero(seat_data[:, 2] > tail_start)[0]
    ...
    for k in range(len(t_idxs)):
        ...
        y_border = rp.max(x_pts * rp.tan(theta2))

        if rp.any(y_pts > y_border):
            removed.append(idx)

    seat_data = rp.delete(seat_data, removed, axis=0)
