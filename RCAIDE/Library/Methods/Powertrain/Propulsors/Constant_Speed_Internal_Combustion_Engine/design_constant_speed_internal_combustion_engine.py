...
def design_constant_speed_internal_combustion_engine(ICE_CS, number_of_stations=20):
    ...
    **Major Assumptions**
    ...
    operating_state       = setup_operating_conditions(ICE_CS,velocity_range=rp.array([V]) , altitude = 0, angle_of_attack=0,temperature_deviation=0)  
    operating_state.conditions.energy.propulsors[ICE_CS.tag].throttle = operating_state.conditions.energy.propulsors[ICE_CS.tag].throttle.at[:,0].set(1.0)
