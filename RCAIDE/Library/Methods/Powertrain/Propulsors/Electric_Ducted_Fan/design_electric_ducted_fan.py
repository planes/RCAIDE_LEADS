...
def design_electric_ducted_fan(EDF, new_regression_results=False, keep_files=True):
    ...
    **Major Assumptions**
    ...
    operating_state       = setup_operating_conditions(EDF,velocity_range=rp.array([V]), altitude = 0, angle_of_attack=0, temperature_deviation=0)  
    operating_state.conditions.energy.propulsors[EDF.tag].throttle = operating_state.conditions.energy.propulsors[EDF.tag].throttle.at[:,0].set(1.0)
