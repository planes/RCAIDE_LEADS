...
def append_bus_segment_conditions(bus,segment):
    ...
    **Major Assumptions**
    ...
    bus_conditions.fuel_mass_flow_rate = bus_conditions.fuel_mass_flow_rate.at[:,0].set(0)
    ...
    if segment.state.initials:
            ...
            for coolant_line in  network.coolant_lines:
                for tag, item in  coolant_line.items():
                    if tag == 'battery_modules':
                        for battery in item:
                            for btms in  battery:
                                bus_conditions.power_draw = bus_conditions.power_draw.at[0,0].add(segment.state.initials.conditions.energy.coolant_lines[coolant_line.tag][btms.tag].power[-1][0])
                    if tag == 'heat_exchangers':
                        for heat_exchanger in  item:                    
                            bus_conditions.power_draw = bus_conditions.power_draw.at[0,0].add(segment.state.initials.conditions.energy.coolant_lines[coolant_line.tag][heat_exchanger.tag].power[-1][0])
        # Bus Properties 
        ...
        bus_conditions.energy = bus_conditions.energy.at[0,0].set(bus_initials.energy[-1,0])
