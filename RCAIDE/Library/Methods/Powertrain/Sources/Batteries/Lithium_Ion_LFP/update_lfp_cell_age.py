...
def update_lfp_cell_age(battery_module, segment, battery_conditions, increment_battery_age_by_one_day):  
    ...
    C_rate             = rp.sqrt(rp.mean(I**2)) /battery_module.cell.nominal_capacity
    
    # Semi Emperical aging model  
    E_fade_factor = 1-(((Temp**2*2.0916e-8)+(-1.2179e-5*Temp)+0.0018)*rp.exp(((-1.7082e-6*Temp)+0.0556)*C_rate) \
                        * charge_thougput + (5.9808e6) * rp.exp(0.68989*SOC) * rp.exp(-6.4647e3/Temp) * t**(0.5))


    battery_conditions.cell.capacity_fade_factor     = rp.minimum(E_fade_factor[-1][0],battery_conditions.cell.capacity_fade_factor)
