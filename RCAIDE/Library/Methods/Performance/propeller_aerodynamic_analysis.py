...
def propeller_aerodynamic_analysis(propeller,
    ...
    conditions.frames.planet.true_course[:,0,0]            = rp.cos(0.0),
    conditions.frames.planet.true_course[:,0,1]            = - rp.sin(0.0)
    conditions.frames.planet.true_course[:,1,0]            = rp.sin(0.0)
    conditions.frames.planet.true_course[:,1,1]            = rp.cos(0.0) 
    conditions.frames.planet.true_course[:,2,2]            = 1 
    conditions.frames.wind.transform_to_inertial           = rp.zeros((ctrl_pts,3,3))   
    conditions.frames.wind.transform_to_inertial[:,0,0]    = rp.cos(0.0) 
    conditions.frames.wind.transform_to_inertial[:,0,2]    = rp.sin(0.0) 
    conditions.frames.wind.transform_to_inertial[:,1,1]    = 1 
    conditions.frames.wind.transform_to_inertial[:,2,0]    = -rp.sin(0.0) 
    conditions.frames.wind.transform_to_inertial[:,2,2]    = rp.cos(0.0)  
    conditions.frames.body.transform_to_inertial           = rp.zeros((ctrl_pts,3,3))
    conditions.frames.body.transform_to_inertial[:,0,0]    = rp.cos(angle_of_attack)
    conditions.frames.body.transform_to_inertial[:,0,2]    = rp.sin(angle_of_attack)
    conditions.frames.body.transform_to_inertial[:,1,1]    = 1
    conditions.frames.body.transform_to_inertial[:,2,0]    = -rp.sin(angle_of_attack)
    conditions.frames.body.transform_to_inertial[:,2,2]    = rp.cos(angle_of_attack)  
