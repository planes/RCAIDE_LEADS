...
def primary_noise_component(Velocity_primary, Temperature_primary, R_gas, theta_p, DVPS, sound_ambient, 
    ...
    **Definitions**
    ...
    sound_primary    = rp.sqrt(1.4*R_gas*Temperature_primary) 
    Mach_primary_jet = Velocity_primary/sound_primary  
    
    # Calculation of the velocity exponent 
    velocity_exponent = 1.5*rp.exp(-10*(theta_p - 2.2)**2)
    velocity_exponent[theta_p <= 2.2] = 1.56

    # Calculation of the Source Strengh Function (FV)
    FV = Mach_primary_jet*(DVPS/sound_ambient)**0.6*((Velocity_primary+Velocity_secondary)/sound_ambient)**0.4*  (rp.abs(Velocity_primary-Velocity_aircraft)/Velocity_primary)**velocity_exponent

    # Determination of the noise model coefficients
    Z1 = -18*((1.8*theta_p/rp.pi)-0.6)**2
    Z2 = -18-18*((1.8*theta_p/rp.pi)-0.6)**2
    Z3 = 0.0
    Z4 = -0.1 - 0.75*((Velocity_primary-Velocity_secondary-Velocity_aircraft)/sound_ambient) *  ((1.8*theta_p/rp.pi)-0.6)**3. + 0.8*(0.6-rp.log10(1+Area_secondary/Area_primary))
    Z5 = 50 + 20*rp.exp(-(theta_p-2.6)**2.)
    Z6 = 94 + 46*rp.exp(-(theta_p-2.5)**2.) - 26.*(0.6-rp.log10(1+Area_secondary/Area_primary))/  rp.exp(5*(theta_p-2.3)**2) + DSPL_p + EX_p

    # Determination of Sound Pressure Level for the primary jet component
    SPL_p = (Z1*rp.log10(FV)+Z2) * (rp.log10(Str_p)-Z3*rp.log10(FV)-Z4)**2 + Z5*rp.log10(FV) + Z6
