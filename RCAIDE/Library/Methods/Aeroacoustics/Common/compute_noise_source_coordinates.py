...
def compute_rotor_point_source_coordinates(rotor,conditions,mls,settings):
    ...
    num_mic     = len(mls[:,0])   
    rot_origins = rp.array(rotor.origin)  
    num_blades  = rotor.number_of_blades  
    num_sec     = len(rotor.radius_distribution)    
    
    # Get the rotation matrix  
    phi         = rp.linspace(0,2*rp.pi,num_blades+1)[0:num_blades]  
    ...
    # -----------------------------------------------------------------------------------------------------------------------------
    I0    = rp.atleast_3d(rp.array([[0,0,0,1]]))
    I0    = rp.array(I0)  
    mat_0 = rp.tile(I0[None,None,None,:,:,:],(num_cpt,num_mic,num_blades,num_sec,1,1))
    ...
    mat_0_te   = rp.matmul(rev_Translation_mic_loc,mat_0)    
    mat_1_te   = rp.matmul(rev_Rotation_AoA,mat_0_te) 
    mat_2_te   = rp.matmul(rev_Translation_origin_to_rel_loc,mat_1_te) 
    mat_3_te   = rp.matmul(rev_Rotation_thrust_vector_angle,mat_2_te) 
    mat_4_te   = rp.matmul(rev_Rotation_blade_azi,mat_3_te) 
    mat_5_te   = rp.matmul(rev_Rotation_blade_flap,mat_4_te)
    mat_6_te   = rp.matmul(rev_Rotation_blade_twist,mat_5_te)   
    mat_7_te   = rp.matmul(rev_Tranlation_blade_trailing_edge,mat_6_te)   
          
    # for points on quarter chord of untwisted blade 
    mat_6_qc = rp.matmul(rev_Tranlation_blade_quarter_chord,mat_5_te)
    
    # for position vector from rotor point to obverver 
    mat_0_p  = mat_0 # rp.matmul(Rotation_thrust_vector_angle,mat_0)
    mat_1_p  = rp.matmul(Translation_origin_to_rel_loc,mat_0_p)
    mat_2_p  = rp.matmul(Rotation_AoA,mat_1_p) 
    mat_3_p  = rp.matmul(Translation_mic_loc,mat_2_p) 
    
    # for position vector from rotor blade section to observer (alligned with wind frame)
    mat_0_bs = rp.matmul(Tranlation_blade_quarter_chord,mat_0)
    mat_1_bs = rp.matmul(Rotation_blade_twist,mat_0_bs)
    mat_2_bs = rp.matmul(Rotation_blade_flap,mat_1_bs)
    mat_3_bs = rp.matmul(Rotation_blade_azi,mat_2_bs)
    mat_4_bs = rp.matmul(Rotation_thrust_vector_angle,mat_3_bs)
    mat_5_bs = rp.matmul(Translation_origin_to_rel_loc,mat_4_bs)
    mat_6_bs = rp.matmul(Rotation_AoA,mat_5_bs)
    mat_7_bs = rp.matmul(Translation_mic_loc,mat_6_bs)   
    ...
    # ----------------------------------------------------------------------------------------------------------------------------- 
    S              = rp.linalg.norm(X, axis =4)     
    S_prime        = rp.linalg.norm(X_prime, axis =4)    
    S_e            = rp.linalg.norm(X_e, axis =4)    
    S_hub          = rp.linalg.norm(X_hub, axis =4)   
    
    theta          = rp.arccos(X[:,:,:,:,0]/S)   
    theta_e        = rp.arccos(X_e[:,:,:,:,0]/S_e)  
    theta_hub      = rp.arccos(X_hub[:,:,:,:,0]/S_hub)
    theta_hub[X_hub[:,:,:,:,1]<0] = ( rp.pi * 2) -  theta_hub[X_hub[:,:,:,:,1]<0] 
     
    M_x            = rp.linalg.norm(M_vec[:,:,:,:,0:3,3], axis =4)  
        
    theta_r        =  rp.arccos(rp.cos(theta)*rp.sqrt(1 - ((M_x**2)*rp.sin(theta)**2 )) + M_x*rp.sin(theta)**2)
    theta_e_r      =  rp.arccos(rp.cos(theta_e)*rp.sqrt(1 - ((M_x**2)*rp.sin(theta_e)**2 )) + M_x*rp.sin(theta_e)**2)
    theta_hub_r    =  rp.arccos(rp.cos(theta_hub)*rp.sqrt(1 - ((M_x**2)*rp.sin(theta_hub)**2 )) + M_x*rp.sin(theta_hub)**2)

    

    Y              = rp.sqrt(X[:,:,:,:,1]**2 + X[:,:,:,:,2]**2)     
    Y_prime        = rp.sqrt(X_prime[:,:,:,:,1]**2 + X_prime[:,:,:,:,2]**2)          
    Y_e            = rp.sqrt(X_e[:,:,:,:,1]**2 + X_e[:,:,:,:,2]**2)            
    Y_hub          = rp.sqrt(X_hub[:,:,:,:,1]**2 + X_hub[:,:,:,:,2]**2)    

    phi_e          = rp.arccos(X_e[:,:,:,:,1]/Y_e) 
    phi_hub        = rp.arctan2(X_hub[:,:,:,:,1], X_hub[:,:,:,:,2])
    ...
    alpha           = rp.tile((conditions.aerodynamics.angles.alpha  + rp.arccos(body2thrust[0,0]))[:,:,None,None], (1,num_mic,num_blades,num_sec ))
    theta_prime_r   = rp.arccos(rp.cos(theta_r)*rp.cos(alpha) + rp.sin(theta_r)*rp.sin(phi_hub)*rp.sin(alpha)) # Eq 3.10 Hanson Sound from a propeller at angle of attack: a new theoretical viewpoint    
 
    x_r            = Y/(rp.tan(theta_r))                             
    x_prime_r      = Y_prime/(rp.tan(theta_prime_r))                             
    x_e_r          = Y_e/(rp.tan(theta_e_r))                             
    x_hub_r        = Y_hub/(rp.tan(theta_hub_r))    

    X_prime_r      = rp.zeros_like(X_prime)  
    X_e_r          = rp.zeros_like(X_e)   
    X_r            = rp.zeros_like(X)   
