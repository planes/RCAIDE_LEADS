...
def compute_fuselage_moment_of_inertia(fuselage,center_of_gravity=[[0, 0, 0]]): 
    ...
    I_total = rp.zeros((3, 3)) # Initialize matrix to hold the entire fuselage inertia tensor
    ...
    # Hemisphere
    # ----------------------------------------------------------------------------------------------------------------------   
    origin_hemisphere = rp.array([fuselage.lengths.nose, 0, 0]) + rp.array(fuselage.origin)
    mass_hemisphere   = mass_fuselage * volume_fraction[0] # mass of the hemisphere
    I                 = rp.zeros((3, 3)) # Local inertia tensor 
    ...
    # Moment of inertia in local system. From Weisstein [2]. 
    rho     = (mass_cone / (1 / 3 * rp.pi * (outer_radius ** 2 * center_length - inner_radius ** 2 * (center_length * inner_radius / outer_radius)))) # density of the cone. Mass divided by volume.
    I[0][0] = rho * (1 / 3 * rp.pi * outer_radius ** 2 * tail_length ** 3 + rp.pi / 20 * outer_radius ** 4 *tail_length - 1 / 3 * rp.pi * inner_radius ** 2 * (tail_length * inner_radius / outer_radius) ** 3 + rp.pi / 20 * inner_radius ** 4 *(tail_length * inner_radius / outer_radius))
    I[1][1] = rho * (1 / 3 * rp.pi * outer_radius ** 2 * tail_length ** 3 + rp.pi / 20 * outer_radius ** 4 *tail_length - 1 / 3 * rp.pi * inner_radius ** 2 * (tail_length * inner_radius / outer_radius) ** 3 + rp.pi / 20 * inner_radius ** 4 *(tail_length * inner_radius / outer_radius))
    I[2][2] = rho * (rp.pi /10 *outer_radius **4 *tail_length -rp.pi /10 *inner_radius **4 *(tail_length * inner_radius / outer_radius)) # Izz
...
def Volume_Fraction(outer_radius, inner_radius, center_length, tail_length):
    ...
    volume_cone       = rp.pi * outer_radius ** 2 * tail_length / 3 - rp.pi * inner_radius ** 2 * (tail_length * inner_radius / outer_radius) / 3
    volume_hemisphere = 2 / 3 * rp.pi * outer_radius ** 3 -2 / 3 * rp.pi * inner_radius ** 3 
    volume_cylinder   = rp.pi * center_length * (outer_radius ** 2 - inner_radius ** 2)
    ...
    # Volume Fractions
    # ----------------------------------------------------------------------------------------------------------------------        
    volume_fraction = rp.array([volume_hemisphere / volume_total, volume_cylinder / volume_total, volume_cone / volume_total])
