...
def compute_fuselage_weight(fuse,maxSpan, MTOW, 
    ...
    try:
        skinArealWeight = rp.sum([(mat.minimum_gage_thickness * mat.density) for mat in fuse.skin_materials])
    except AttributeError:
        skinArealWeight = 1.2995 # Stack of bidirectional CFRP, Honeycomb Core, Paint

    try:
        canopyArealWeight = rp.sum([(mat.minimum_gage_thickness * mat.density) for mat in fuse.canopy_materials])
    except AttributeError:
        canopyArealWeight = 3.7465 # Acrylic

    # Calculate fuselage area (using assumption of ellipsoid), and weight:

    S_wet = 4 * rp.pi * (((fLength * fWidth/4)**1.6
    ...
    bulkheadMass = 3 * rp.pi * fHeight * fWidth/4 * skinArealWeight
