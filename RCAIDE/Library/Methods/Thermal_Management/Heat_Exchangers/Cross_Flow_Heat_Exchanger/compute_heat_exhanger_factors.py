...
def compute_heat_exhanger_factors(kc_vals,ke_vals,delta, Re):
    ...
    for i in range(Kc.shape[0]):
        if Re[i] < 2000:
            ...
            Kc_set_T = rp.array([(kc_vals[:, 2]), (kc_vals[:, 3]), (kc_vals[:, 4]), (kc_vals[:, 5])])
            f = interpolate.interp2d(delta_set, Re_set, Kc_set_T, kind='linear')
            Kc[i] = f(delta, Re[i])
    
    Ke = rp.zeros(shape=Re.shape)
    for i in range(Ke.shape[0]):
        if Re[i] < 2000:
            ...
            delta_set = ke_vals[:, 0]
            Re_set = [2e3, 3e3, 5e3, 1e4]
            Ke_set_T = rp.array([(ke_vals[:, 2]), (ke_vals[:, 3]), (ke_vals[:, 4]), (ke_vals[:, 5])])
