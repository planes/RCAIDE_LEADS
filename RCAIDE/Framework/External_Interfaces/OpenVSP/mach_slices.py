...
def mach_slices(vehicle,mach,angle_of_attack=[0.],number_slices = 99):
    ...
    for ii in range(len(mach)):
        ...
        mach_angle = rp.arcsin(1/m)
        roty = (rp.pi/2-mach_angle) + aoa
        
        # Take the components of the X and Z axis to get the slicing plane
        x_component = rp.cos(roty)
        z_component = rp.sin(roty)
        ...
        slice_areas    = vsp.GetDoubleResults( pslice_results, "Slice_Area" ) * rp.cos(roty)
        ...
        X_locs      = rp.array(X_locs)
        slice_areas = rp.array(slice_areas)
